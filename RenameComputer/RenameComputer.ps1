...
if ($goodToGo) {
    ...
    if ($assetTag.Length -gt 9) {
        $assetTag = $assetTag.Substring(0, 9)
    }
    $CNYear = ((Get-Date).Year).ToString().Substring(2, 2) 
    if ($details.CsPCSystemTypeEx -eq "Mobile") {
        $newName = "CSN-$assetTag-$CNYear"
    }
    else {
        $newName = "CSD-$assetTag-$CNYear"
    }

    # Is the computer name already set?  If so, bail out
    if ($($newName.Substring(0, 9)) -ieq $($details.CsName).Substring(0, 9)) {
