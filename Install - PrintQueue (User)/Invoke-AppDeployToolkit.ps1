...
$adtSession = @{
    # App variables.
    AppVendor                   = ''
    AppName                     = 'Install Network Printer'
    AppVersion                  = ''
    AppArch                     = ''
    AppLang                     = 'EN'
    AppRevision                 = '01'
    ...
    AppScriptAuthor             = 'Chris Chisholm'
...
#Define Print Server Name
$PrintServerName = 'printsrv001.contoso.com'
#Define Printer Name
$PrinterName = 'Test Print'
#Define Print Server Path
$PrinterPath = "\\$PrintServerName\$PrinterName"
...
function Install-ADTDeployment {
    ...
    if ($PrinterInstalled) {
        Write-ADTLogEntry -Message "Printer [$PrinterName] is already installed." -Source 'Add-Printer'
    }
    else {
        if (!(Test-Connection -ComputerName $PrintServerName -Count 2 -Quiet)) {
            Write-ADTLogEntry -Message "Print server [$PrintServerName] is not reachable. Cannot add printer [$PrinterName]." -Severity 3 -Source 'Add-Printer'
        }
        else {
            Write-ADTLogEntry -Message "Print server [$PrintServerName] is reachable." -Source 'Add-Printer'
            #Add the Printer
            
            try {
                Add-Printer -ConnectionName $PrinterPath -ErrorAction Stop
                Write-ADTLogEntry -Message "Successfully added printer [$PrinterName] from path [$PrinterPath]." -Source 'Add-Printer'
            }
            catch {
                Write-ADTLogEntry -Message "Failed to add printer [$PrinterName] from path [$PrinterPath]. Error: $_" -Severity 3 -Source 'Add-Printer'
            }
        }
    }
...
function Uninstall-ADTDeployment {
    ...
    #how-ADTInstallationWelcome -CloseProcesses iexplore -CloseProcessesCountdown 60
    ...
    Get-Printer -Name $PrinterPath -ErrorAction SilentlyContinue | ForEach-Object {
        Write-ADTLogEntry -Message "Removing printer [$($_.Name)]." -Source 'Remove-Printer'
        try {
            Remove-Printer -Name $($_.Name)
            Write-ADTLogEntry -Message "Successfully removed printer [$($_.Name)]." -Source 'Remove-Printer'
        }
        catch {
            Write-ADTLogEntry -Message "Failed to remove printer [$($_.Name)]. Error: $_" -Severity 3 -Source 'Remove-Printer'
        }
    }
